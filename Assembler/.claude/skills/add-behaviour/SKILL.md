...
- **Be pooling-safe (issue #102).** A spawned entity's GameObject and all its components are *reused* when it is destroyed and a same-template entity is later spawned. The build pipeline re-runs `OnInitialise` on the existing component for the new spawn, and there is no second `Awake`. So:
  - **Create Unity sub-components once, guarded.** If `OnInitialise` `AddComponent`s a Unity component (Rigidbody, renderer, collider, AudioSource…) or spawns a child GameObject, guard it (`if (_thing == null) _thing = …`) and reuse the field — an unguarded create stacks a duplicate on every reuse. Do **not** move it to `Awake`: `Awake` does not run in edit mode, so the sandbox validator (`validate-game`) and EditMode tests would never create it. (See `RigidbodyBehaviour`, `SpriteBehaviour`, `AddColliderBehaviour`.)
  - **Reset transient state in `OnReuse()`.** Override `public override void OnReuse()` to clear private mutable state that would otherwise leak from the previous life — counters, debounce timestamps, gesture flags, cached velocities — and to **re-arm any one-shot `Start()` logic** (auto-start timers, scan coroutines, initial state-machine hooks), since `Start` does not re-fire on a reused component. `OnReuse` runs *after* re-initialisation, so it sees this spawn's `Data`; teardown of something `OnInitialise` itself re-creates (a live subscription, input wiring) belongs in an idempotent `OnInitialise` instead. A stateless behaviour needs nothing.
