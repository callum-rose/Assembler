...
Favour modern C# and a functional style. These are preferences, not absolutes — break them when clarity demands it. The mechanical parts (indentation, brace placement, wrapping, spacing) are enforced by **CSharpier** via the repo `.editorconfig` — run `Tools/check-format.sh` to verify, or `Tools/check-format.sh --fix` to auto-format. The IDE/analyzer style rules in `.editorconfig` surface the higher-level preferences below as inline suggestions.
...
- **Check formatting**: run `Tools/check-format.sh` to verify C# matches house style. Unlike the other scripts it does **not** boot Unity — it runs **CSharpier** (pinned in `.config/dotnet-tools.json`, restored on first run) standalone over the `.cs` files, so it's fast (~1s) and needs only the .NET SDK. CSharpier owns mechanical formatting (indentation, braces, wrapping, spacing); the repo `.editorconfig` supplies the tab indent / LF / 120-col print width and `.csharpierignore` excludes vendored code (`Assets/Plugins`, `Assets/TextMesh Pro`). **Default:** checks only the `.cs` files changed vs `master` (committed, staged, unstaged, or untracked) — so you can confirm your own change is clean before committing. Pass `--all` to check every file under `Assets/`, explicit file/dir paths to scope it, or `--fix` to format in place instead of checking. Exits non-zero if anything needs reformatting.
