...
Deterministic execution and record/replay (same descriptor + seed + input log → identical run) is intended to make generated games debuggable (capture a session, replay it exactly) and testable (play a descriptor through a scripted input log and assert on the outcome). All three pieces are now in place — a selectable fixed-step clock, a seeded per-run PRNG, and input record/replay at the trigger boundary (issue #101) — so a **non-physics** descriptor built under `FixedStep` + a fixed seed and replayed from a captured input log reproduces its run. The **guaranteed scope is Level 1 (same build, same machine), with physics-driven games excluded**; cross-platform lockstep is a non-goal (floating-point/physics differ across CPUs/OSes/Unity versions). Per-frame behaviours now step in one defined order and tweens/physics forces are clock-aligned (#241, see items 6–7 below), so the shared-velocity movement stack replays and animations pause with the game; the remaining ordering nuance is limited to trigger-chain resumptions within a tick.
...
5. **End-to-end replay proof** — `Tests/Determinism/ReplayDeterminismTests.cs` (**PlayMode**) records an input log on a physics-free descriptor (embedded as a YAML string in the test, so it also loads in a built player) under `FixedStep` + a fixed seed — driving a synthetic device through the new **`auto` controls scheme** (`InputPlatform.Auto`, override-only, masks in a descriptor's `auto` binding group) — then replays the captured log on a fresh build and asserts the mover's final transform matches, plus a negative check that a different seed diverges. A third case (`Shared_velocity_stack_replays_deterministically`) puts the shared-velocity stack under replay, and `ClockDrivenTweenTests` proves a tween freezes/resumes with the game clock. Verified headlessly via `Tools/run-tests.sh --player Tests.Determinism`.
6. **Deterministic per-frame ordering (#241)** — every per-frame behaviour (the 15 `PerFrameBehaviour` subclasses: velocity, acceleration, drag, speed limit, steering, patrol, FSM, …) is stepped by a single `PerFrameDriver` on the game root (`Assets/Behaviours/PerFrameDriver.cs`, `[DefaultExecutionOrder(-8000)]` — after the clock tick and replayed input, before everything else) in `Assembler.Building.BehaviourRegistry` registration (descriptor) order, rather than each driving itself off its own `Update` in Unity's undefined per-component order. This makes the shared-velocity stack (acceleration → drag → speed limit → velocity, all mutating one `!var velocity`) compose in the documented order and reproduce on replay. Behaviours expose the tick through the `IPerFrameStepped` interface; the driver reads the registry's live list, so entities spawned/destroyed mid-run stay in sync via the registry's register/deregister path. The driver keeps the old clock-gating (steps once per *advanced* frame, skipped while paused, advanced by a debug step). Covered by the velocity-stack probe in `Tests/Determinism/ReplayDeterminismTests.cs`.
7. **Clock-driven tweens (#241)** — `AnimationBehaviour`'s DOTween sequences are marked `UpdateType.Manual` and pumped by a `TweenDriver` on the game root (`Assets/Behaviours/Animations/TweenDriver.cs`, `[DefaultExecutionOrder(-7900)]`) via `DOTween.ManualUpdate(Clock.DeltaTime, Clock.UnscaledDeltaTime)`, instead of DOTween's own wall-clock loop. So tweens freeze at `set timescale 0` (their `OnComplete`, which fires listener/trigger chains — i.e. game logic — no longer fires into a paused game), honour the clock's timescale, and under `FixedStep` advance by a constant step so tween-completion timing replays. Also fixed alongside: **`add force`/`add torque`** now apply an impulse scaled by the step delta (`RigidbodyGameBehaviour.StepDelta` — the fixed timestep inside a physics step, else `Clock.DeltaTime`) instead of a raw `ForceMode.Force`, so a per-frame trigger's thrust no longer scales with the refresh rate; driven from a fixed-update/collision trigger it is mathematically identical to the old behaviour.

**Known carve-outs (in-scope work still open):**

- **Same-tick *trigger* ordering (#241, remaining)** — per-frame *behaviours* now step in a defined order (item 6) and tweens/forces are clock-aligned (item 7), so the movement/velocity stack replays and animations pause with the game. What remains undefined is the order in which multiple *triggers* resume/fire within one tick (e.g. two timers elapsing on the same frame), so a descriptor whose outcome depends on that can still diverge on replay.
...
- **PlayMode headless runner** — run PlayMode tests with **`Tools/run-tests.sh --player <assembly>`**, which builds a StandaloneOSX player and runs them inside it (Unity's `-runTests`). This is the reliable headless path and doesn't hold the project path, so it runs alongside other worktrees. The in-editor batch path (`--playmode` / `Editor.TestBatch.RunPlayModeTests`) still **hangs on the play→edit transition** on this Unity setup — kept for the in-editor Test Runner, but prefer `--player` for headless/CI. (Player tests must not depend on `Application.dataPath` file layout — e.g. `ReplayProbe` is an embedded string, not a loose asset.)
