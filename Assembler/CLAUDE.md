...
2. **A seeded per-run PRNG** — `RandomMath` (`Assets/Libraries/RandomMath.cs`) draws every helper from a single ambient `DeterministicRng` (`Assets/Libraries/DeterministicRng.cs`, a self-contained PCG32 struct — no `UnityEngine.Random`, no `Unity.Mathematics` dependency), not the engine's global RNG. `Builder.Instantiate` calls `RandomMath.Seed(uint)` once per run from `RunOptions.Seed` (an explicit seed for a deterministic/replay run; otherwise one derived from entropy so normal play still varies each launch) and logs the resolved seed. `SteeringMath.Wander` routes through `RandomMath` too. The generator is **static/ambient**, so it assumes one game runs at a time (fine at Level 1). Same seed → identical draw sequence; covered by `Tests/Resolving/DeterministicRngTests.cs` and the seed-reproducibility tests in `RandomMathTests`.
