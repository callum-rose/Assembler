...
Deterministic execution and record/replay (same descriptor + seed + input log → identical run) is a **design goal** for Assembler, intended to make generated games debuggable (capture a session, replay it exactly) and testable (play a descriptor through a scripted input log and assert on the outcome). **It is only partially implemented today — there is currently no end-to-end determinism guarantee.** The **target scope is Level 1 (same build, same machine), with physics-driven games excluded** (issue #101); cross-platform lockstep is a non-goal (floating-point/physics differ across CPUs/OSes/Unity versions). The two foundational sources — a selectable fixed-step clock and a seeded per-run PRNG — are now in place; **replay infrastructure (input record/replay) does not yet exist**, so don't assume it. The relevant types below are noted as present or missing.
...
- **End-to-end replay test** — the fixed-step clock and seeded PRNG are unit-tested, but there is no full game-replay regression test (needs PlayMode, since behaviour `Update` doesn't run in EditMode).

**To complete the full guarantee**, the remaining shape is: record/replay at the trigger boundary (`InputTrigger.NotifyListeners` → `TriggerContext`) capturing the ordered `(trigger, emitted context)` set per tick, keyed to the fixed-delta clock and seed captured in `RunOptions` (both now in place, above). The target stays **Level 1 (same build, same machine)** only. Physics-driven games remain excluded (Unity's `PhysicsScene` stepping isn't controlled here; manual `Physics.Simulate` is future work).
