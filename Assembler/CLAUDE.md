...
- **Headless PlayMode runner** — `Tools/run-tests.sh --playmode` (and the underlying `Editor.TestBatch.RunPlayModeTests`) exists, but batch-mode PlayMode execution currently hangs on the play→edit transition on this Unity setup, so the end-to-end test can't be verified headlessly yet — run `ReplayDeterminismTests` from the in-editor **Window > General > Test Runner** (PlayMode). Tracked as a follow-up; the EditMode session tests (point 4) do run headlessly.
