...
Deterministic execution and record/replay (same descriptor + seed + input log → identical run) is a **design goal** for Assembler, intended to make generated games debuggable (capture a session, replay it exactly) and testable (play a descriptor through a scripted input log and assert on the outcome). **It is only partially implemented today — there is currently no end-to-end determinism guarantee.** The **target scope is Level 1 (same build, same machine), with physics-driven games excluded** (issue #101); cross-platform lockstep is a non-goal (floating-point/physics differ across CPUs/OSes/Unity versions). Do not write code that assumes a seeded RNG or replay infrastructure exists; the relevant types below are noted as present or missing.

**What is in place today:**

1. **A clock abstraction with a selectable deterministic clock** — `IGameClock` (`Assets/Time/`) is threaded through time-dependent behaviours and triggers (they read `Clock.DeltaTime` / `WaitForGameSeconds`, not `Time.deltaTime`). Two clocks implement it: `RealtimeGameClock` (wall-clock delta, the default) and **`FixedStepGameClock`** (constant `StepSeconds` per tick, ignoring wall-clock — the deterministic clock). Both implement the driver-facing `IAdvancingGameClock` seam (`Tick()` + `CaptureDeltaTime`), kept off `IGameClock` so consumers can't advance time and test fakes stay minimal. `Builder.Instantiate(GameClockMode)` selects between them (**defaults to `Realtime`**; deterministic/replay runs pass `FixedStep`). Under `FixedStep`, `GameClockDriver` also sets `UnityEngine.Time.captureDeltaTime` to the step (restored to 0 on teardown) so Unity's frame cadence and physics accumulator march at the same constant step. Unit-covered by `Tests/Behaviours/FixedStepGameClockTests.cs`.
...
- **Seeded PRNG (`RandomState` / `DeterministicRng`)** — do not exist. `RandomMath` (`Assets/Libraries/RandomMath.cs`) calls `UnityEngine.Random` directly (the engine's **global, uncaptured** state), so randomness is **not** currently reproducible. A descriptor that draws from `RandomMath` will not replay identically even under `FixedStep`.
- **Input record/replay** — there is no recorder/replay or input-log capture at the trigger boundary.
- **End-to-end replay test** — the fixed-step clock is unit-tested, but there is no full game-replay regression test (needs PlayMode, since behaviour `Update` doesn't run in EditMode, and a no-RNG descriptor).

**To complete the full guarantee**, the remaining shape is: a single seeded per-run PRNG that `RandomMath` routes through (replacing the direct `UnityEngine.Random` calls), and record/replay at the trigger boundary (`InputTrigger.NotifyListeners` → `TriggerContext`) capturing the ordered `(trigger, emitted context)` set per tick — the fixed-delta clock they build on is now in place (above). The target stays **Level 1 (same build, same machine)** only. Physics-driven games remain excluded (Unity's `PhysicsScene` stepping isn't controlled here; manual `Physics.Simulate` is future work).
