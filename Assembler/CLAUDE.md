...
Deterministic execution and record/replay (same descriptor + seed + input log → identical run) is intended to make generated games debuggable (capture a session, replay it exactly) and testable (play a descriptor through a scripted input log and assert on the outcome). All three pieces are now in place — a selectable fixed-step clock, a seeded per-run PRNG, and input record/replay at the trigger boundary (issue #101) — so a **non-physics** descriptor built under `FixedStep` + a fixed seed and replayed from a captured input log reproduces its run. The **guaranteed scope is Level 1 (same build, same machine), with physics-driven games excluded**; cross-platform lockstep is a non-goal (floating-point/physics differ across CPUs/OSes/Unity versions). Per-frame behaviours now step in one defined order (#241, see item 6 below), so the shared-velocity movement stack replays; the remaining ordering nuance is limited to trigger-chain resumptions within a tick (and tweens/physics forces, the other halves of #241).
...
- **Same-tick *trigger* ordering (#241, partial)** — per-frame *behaviours* now step in a defined order (item 6), so the movement/velocity stack replays. What remains undefined is the order in which multiple *triggers* resume/fire within one tick (e.g. two timers elapsing on the same frame), so a descriptor whose outcome depends on that can still diverge. The other two halves of #241 are also open: **tweens** (DOTween runs on Unity wall-clock, so `OnComplete`→`NotifyListeners` fires into a paused game and isn't clock-driven for replay) and **frame-rate-dependent physics forces** (`AddForce`/`AddTorque` with `ForceMode.Force` from Update-rate triggers). Physics is excluded from the determinism guarantee regardless (below).
