...
namespace Assembler.Parsing.Phase2.Parsing.Phase2.Info
{
	...
	public record SphereColliderInfo(string Id, ValueWrapper<float> Size) : BehaviourInfo(Id);
	public record RigidbodyInfo(string Id, ValueWrapper<bool> UseGravity) : BehaviourInfo(Id);
	public record VelocityInfo(string Id, ValueWrapper<string> VelocityVariableId) : BehaviourInfo(Id);
	public record TranslateInfo(string Id, ValueWrapper<Vector3> Displacement) : BehaviourInfo(Id);
	public record KeyHoldTriggerInfo(string Id, ValueWrapper<string> Key, IReadOnlyList<ListenerInfo> Listeners) : BehaviourInfo(Id);
	public record KeyDownTriggerInfo(string Id, ValueWrapper<string> Key, IReadOnlyList<ListenerInfo> Listeners) : BehaviourInfo(Id);
	public record KeyUpTriggerInfo(string Id, ValueWrapper<string> Key, IReadOnlyList<ListenerInfo> Listeners) : BehaviourInfo(Id);
	...
	public record AfterInfo(string Id, ValueWrapper<float> Delay) : BehaviourInfo(Id);
	public record EveryInfo(string Id, ValueWrapper<float> Interval) : BehaviourInfo(Id);
