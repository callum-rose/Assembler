...
- **All dimensions are voxel counts** (integers); there is no metres/unit scale. One voxel = one world unit at runtime.
...
|---|---|---|
| 0 manifest | `ManifestGenerator` | Brief → set manifest: one asset per distinct *thing*, full bounding box (voxels) + binding `description` per asset. |
...
- **Manifest** (`SetManifest`/`ManifestYaml`): `game`, per asset `id`, `description` (binding theming distilled from the brief), `height`/`length`/`width` (voxels; `length`/`width` 0 = unconstrained), `tolerance`, `symmetry`, `rig`, optional `reference` (image filename, resolved by `IReferenceImageSource` against the window's image folder; missing file silently degrades to no-image).
...
- **Terminal failure reasons must be logged synchronously, not via `Progress<T>`.** In `VoxelSetReviewWindow`, progress is delivered through `Progress<T>`, which marshals each callback asynchronously to the main-thread sync context. At end-of-run the synchronous unwind (`StoreResult` → `Log("Batch complete")` → `ExportSessionLog`) completes *before* the queued progress callbacks drain, so a `"{asset}: FAILED — {ex}"` line posted via `progress.Report` shows in the live GUI but never reaches the saved `session.log` (the file ends on a bare `"{asset}: Failed"` from the synchronous `StoreResult`). Per-attempt reports emitted *during* a run are fine — editor ticks drain the queue between LLM calls; only the final unwind loses them. Anything that must land in the saved log has to be logged synchronously from a main-thread chokepoint like `StoreResult`. (The headless `VoxelizeBatch` path uses synchronous `Debug.Log` and isn't affected.)
