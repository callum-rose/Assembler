...
namespace Assembler.AssetGeneration.MeshToVoxels
{
	...
	public static class ObjToVoxConverter
	{
		...
		/// <summary>
		/// Voxelizes a pre-<see cref="LoadScene">loaded</see> model at a target longest-axis dimension.
		/// Pure CPU work (g3 geometry + managed texture sampling), so it is safe to run on a background thread.
		/// </summary>
		public static VoxResult Convert(LoadedModel model, int maxDimVoxels, IProgressReporter progress)
		{
			...
			double maxDim = model.Mesh.GetBounds().MaxDim;
			if (maxDim <= 0)
			{
				throw new InvalidDataException("Mesh has zero extent; nothing to voxelize.");
			}

			return ConvertCore(model, maxDim / maxDimVoxels, MaxGridDim, progress);
		}

		/// <summary>
		/// Voxelizes at an explicit world-space <paramref name="voxelSize"/> instead of deriving it from a
		/// target dimension — used to bake a <i>lossless master</i> at a voxel-style mesh's native baked
		/// pitch (see <see cref="NativePitch"/>). The grid origin is anchored at the mesh's min corner,
		/// which is itself a vertex and therefore on the baked lattice, so each master voxel centre lands
		/// on a baked voxel centre. If the requested pitch would push the longest axis past
		/// <paramref name="maxGridDim"/>, the pitch is coarsened to fit and a warning is logged.
		/// </summary>
		public static VoxResult ConvertAtVoxelSize(
			LoadedModel model, double voxelSize, IProgressReporter progress, int maxGridDim = MaxGridDim)
		{
			if (voxelSize <= 0)
			{
				throw new ArgumentOutOfRangeException(nameof(voxelSize), "Voxel size must be positive.");
			}
			return ConvertCore(model, voxelSize, Mathf.Max(1, maxGridDim), progress);
		}

		private static VoxResult ConvertCore(
			LoadedModel model, double voxelSize, int maxGridDim, IProgressReporter progress)
		{
			...
			// Keep the longest axis within the grid cap: if the requested pitch is finer than the cap
			// allows, coarsen it to fit. A transient dense bool[]+Color32[] at 256³ is ~80 MB; finer needs care.
			double minVoxelSize = maxDim / maxGridDim;
			if (voxelSize < minVoxelSize)
			{
				Debug.LogWarning(
					$"[MeshToVoxels] Requested voxel size {voxelSize:G4} would exceed the {maxGridDim}³ grid cap; " +
					$"coarsening to {minVoxelSize:G4}.");
				voxelSize = minVoxelSize;
			}

			int gridX = GridDim(b.Width, voxelSize, maxGridDim);
			int gridY = GridDim(b.Height, voxelSize, maxGridDim);
			int gridZ = GridDim(b.Depth, voxelSize, maxGridDim);
		...
		private static int GridDim(double extent, double voxelSize, int maxGridDim) =>
