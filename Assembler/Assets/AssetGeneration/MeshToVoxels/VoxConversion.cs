...
namespace Assembler.AssetGeneration.MeshToVoxels
{
	...
	public static class VoxConversion
	{
			...
			Action<string, float>? pipelineProgress)
		{
			...
			VoxResult result = voxModel.ToResult();
		...
		/// <summary>
		/// Builds the dense working model the pipeline runs over, picking one of two voxelization paths:
		/// <list type="bullet">
		/// <item><b>Native-pitch master (A2)</b> — when enabled <i>and</i> the mesh is confidently grid-like
		/// (a baked voxel lattice), voxelize a lossless master at that native pitch and resample it to the
		/// target <i>in voxel space</i>, so non-power-of-2 targets stay crisp. See <see cref="NativePitch"/>.</item>
		/// <item><b>Direct / supersample-and-downres</b> — the fallback for smooth/organic meshes (and when
		/// the master path is off): voxelize at <c>factor×</c> the target and block-downres, or directly at
		/// the target when <c>factor == 1</c>. Left untouched so it remains a clean A/B against the master path.</item>
		/// </list>
		/// </summary>
		private static VoxModel BuildWorkingModel(
			ObjToVoxConverter.LoadedModel model,
			int maxDimVoxels,
			VoxPipelineSettings settings,
			IProgressReporter voxelProgress,
			Action<string, float>? pipelineProgress)
		{
			if (settings.nativePitchMaster)
			{
				NativePitch.PitchEstimate pitch = NativePitch.Detect(model, settings.nativePitchConfidence);
				if (pitch.Detected)
				{
					pipelineProgress?.Invoke($"Master @ pitch {pitch.Pitch:G3}", 0f);
					VoxResult masterResult = ObjToVoxConverter.ConvertAtVoxelSize(model, pitch.Pitch, voxelProgress);
					VoxModel master = VoxModel.FromResult(masterResult);

					pipelineProgress?.Invoke("Resample", 0f);
					return VoxResample.ToTargetMaxDim(master, maxDimVoxels, new VoxResample.Options(
						settings.downresCoverageThreshold,
						settings.downresFeatureAware,
						settings.downresColourSalience));
				}
				// Not grid-like (e.g. a Method-2 smooth mesh) — fall through to the existing path.
			}
