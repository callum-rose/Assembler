...
namespace Assembler.AssetGeneration.MeshToVoxelSpike.Generation
{
    ...
    public sealed class ModelConfigGenerator
    {
        ...
        public ModelConfigGenerator(AnthropicClient client, StyleRules? rules = null)
        {
            _client = client;
            _rules = rules ?? StyleRules.Load();
            _systemPrompt = ConfigPromptBuilder.Build(_rules);
        }
        ...
        public async Task<ModelConfig> ChooseAsync(string assetDescription, string artContext, CancellationToken ct)
        {
            ...
            return ConfigParser.Parse(raw, _rules);
        }
