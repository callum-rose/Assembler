namespace Assembler.AssetGeneration.MeshToVoxelSpike
{
    ...
    public readonly struct OklabColor
    {
        ...
        public static OklabColor FromColor32(Rgba32 c)
        {
        ...
        public float Chroma => FMath.Sqrt(A * A + B * B);
        ...
        public float DistanceTo(OklabColor other) => FMath.Sqrt(SquaredDistanceTo(other));

        private static float SrgbToLinear(float c) =>
            c <= 0.04045f ? c / 12.92f : FMath.Pow((c + 0.055f) / 1.055f, 2.4f);

        private static float Cbrt(float x) =>
            x < 0f ? -FMath.Pow(-x, 1f / 3f) : FMath.Pow(x, 1f / 3f);
    }
}
