...
namespace Assembler.AssetGeneration.MeshToVoxel.Editor
{
    ...
    /// </summary>
    public static class StagePreviewer
    {
        private const string ParentName = "MeshToVoxel Preview";
        private const float Gap = 0.5f; // world-unit gap between stages, relative to mesh size

        public static void Show(StageResult result, float rowSpacingScale)
        {
            ClearPrevious();
            ...
            Debug.Log(
                $"[MeshToVoxel] Preview built: {result.VoxelCount:N0} voxels " +
        ...
        /// </summary>
        public static GameObject BuildRow(Transform parent, StageResult result, float rowSpacingScale)
        {
        ...
        public static void ShowBlockyOnly(StageResult result)
        {
            ClearPrevious();
            var parent = new GameObject(ParentName);
            CreateStage(parent.transform, "Blocky Voxel Model", result.Blocky, ColouredMaterial(), 0f, result.Blocky.bounds);
            Debug.Log(
                $"[MeshToVoxel] Blocky model built: {result.VoxelCount:N0} voxels " +
                $"({result.GridX}×{result.GridY}×{result.GridZ}). See '{ParentName}' in the scene.");
        }
        ...
        private static Material ColouredMaterial()
        {
            ...
            return new Material(shader) { name = "VertexColour" };
        }
        ...
        {
            var mat = new Material(Shader.Find("Assembler/VertexColorUnlit")) { name = "Grey" };
