...
namespace Assembler.AssetGeneration.MeshToVoxel
{
    ...
    public readonly struct VoxCell
    {
        ...
        public VoxCell(int x, int y, int z, Rgba32 color)
        {
            X = x;
            Y = y;
            Z = z;
    ...
    /// <summary>Grid dims + filled cells for <see cref="VoxWriter"/>.</summary>
    public sealed class VoxResult
    {
        ...
        public VoxResult(int gridX, int gridY, int gridZ, IReadOnlyList<VoxCell> cells)
        {
            GridX = gridX;
            GridY = gridY;
            GridZ = gridZ;
    ...
    /// <summary>
    /// Minimal MagicaVoxel <c>.vox</c> (version 150) binary writer — engine-free (pure
    /// <c>System.IO</c>), so it runs headlessly / outside Unity.
    ///
    ...
    /// otherwise it falls back to median-cut quantisation.
    ///
    ...
    public static class VoxWriter
    {
        ...
        private static int ColorKey(Rgba32 c) => (c.r << 16) | (c.g << 8) | c.b;
        ...
        /// more than 255 distinct colours. Derives the palette from the model's <i>actual</i>
        /// colours: each box's representative is the population-weighted average of its members.
        /// Returns the 256-entry RGBA table plus a colour→slot map. Slots start at
        /// <see cref="FirstColorIndex"/> (slot 1 is the reserved dummy).
        /// </summary>
