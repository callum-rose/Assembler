...
{
    /// <summary>
    /// The engine-free core of the mesh → voxel pipeline: fine SDF → fine-grid analysis → scored
    ...
    /// the editor window's stage-by-stage preview.
    /// </summary>
    public static class MeshVoxeliser
    {
        ...
        private static Stage RunVoxelStage(LoadedModel model, Settings settings, Action<float, string>? progress)
        {
            ...
            var result = new VoxelResult
            {
                ...
                Metrics = VoxelMetrics.Compute(occupancy, voxelColours, placement, floatersRemoved, fineDims),
            };
