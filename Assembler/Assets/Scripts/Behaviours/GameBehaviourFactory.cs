...
namespace Assembler.Behaviours
{
	...
	{
		private delegate (GameBehaviour, Action<IReadOnlyDictionary<BehaviourDescriptor, GameBehaviour>>) BehaviourBuilder(
			GameObject go, BehaviourInfo info, VariableRegistry vr, CompiledExpressionsRegistry cr, IEntitySpawner spawner);

		private static readonly Dictionary<Type, BehaviourBuilder> Builders = new()
		{
			[typeof(BoxColliderInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (BoxColliderInfo)info;
				var b = go.AddComponent<AutoAddBoxColliderBehaviour>();
				return (b, lr => b.Initialise(new BoxColliderData(i.Id, i.Listeners.ToActions(lr),
					i.Size.Resolve(vr, cr), i.IsTrigger.Resolve(vr, cr))));
			},

			[typeof(SphereColliderInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (SphereColliderInfo)info;
				var b = go.AddComponent<AutoAddSphereColliderBehaviour>();
				return (b, lr => b.Initialise(new SphereColliderData(i.Id, i.Listeners.ToActions(lr),
					i.Radius.Resolve(vr, cr))));
			},

			[typeof(RigidbodyInfo)] = (go, info, vr, cr, _) =>
			{
				...
				return (b, lr => b.Initialise(new RigidbodyData(i.Id, i.Listeners.ToActions(lr))
					{ UseGravity = i.UseGravity.Resolve(vr, cr) }));
			},

			[typeof(VelocityInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (VelocityInfo)info;
				var b = go.AddComponent<Velocity>();
				return (b, lr => b.Initialise(new VelocityData(i.Id, i.Listeners.ToActions(lr),
					i.Velocity.Resolve(vr, cr))));
			},

			[typeof(TranslateInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (TranslateInfo)info;
				var b = go.AddComponent<Translate>();
				return (b, lr => b.Initialise(new TranslateData(i.Id, i.Listeners.ToActions(lr),
					i.Displacement.Resolve(vr, cr))));
			},

			[typeof(SetPositionInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (SetPositionInfo)info;
				var b = go.AddComponent<SetPosition>();
				return (b, lr => b.Initialise(new SetPositionData(i.Id, i.Listeners.ToActions(lr),
					i.ValueExpression.Resolve(vr, cr))));
			},

			[typeof(KeyHoldTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (KeyHoldTriggerInfo)info;
				var b = go.AddComponent<KeyHoldTrigger>();
				return (b, lr => b.Initialise(new KeyHoldTriggerData(i.Id, i.Key.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(KeyDownTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (KeyDownTriggerInfo)info;
				var b = go.AddComponent<KeyDownTrigger>();
				return (b, lr => b.Initialise(new KeyDownTriggerData(i.Id, i.Key.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(KeyUpTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (KeyUpTriggerInfo)info;
				var b = go.AddComponent<KeyUpTrigger>();
				return (b, lr => b.Initialise(new KeyUpTriggerData(i.Id, i.Key.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(TapTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(DoubleTapTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(LongPressTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(SwipeTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(DragTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(PinchTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(RotateTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(OnStartTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(TimerTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (TimerTriggerInfo)info;
				var b = go.AddComponent<TimerTrigger>();
				return (b, lr => b.Initialise(new TimerTriggerData(i.Id, i.Delay.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(IntervalTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (IntervalTriggerInfo)info;
				var b = go.AddComponent<IntervalTrigger>();
				return (b, lr => b.Initialise(new IntervalTriggerData(i.Id, i.Interval.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(EveryFrameTriggerInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(CollisionEnterTriggerInfo)] = (go, info, _, _, _) =>
			{
				var i = (CollisionEnterTriggerInfo)info;
				var b = go.AddComponent<CollisionEnter>();
				return (b, lr => b.Initialise(new CollisionEnterTriggerData(i.Id, i.TagsToDetect,
					i.Listeners.ToActions(lr))));
			},

			[typeof(CollisionExitTriggerInfo)] = (go, info, _, _, _) =>
			{
				var i = (CollisionExitTriggerInfo)info;
				var b = go.AddComponent<CollisionExit>();
				return (b, lr => b.Initialise(new CollisionExitTriggerData(i.Id, i.TagsToDetect,
					i.Listeners.ToActions(lr))));
			},

			[typeof(CollisionStayTriggerInfo)] = (go, info, _, _, _) =>
			{
				var i = (CollisionStayTriggerInfo)info;
				var b = go.AddComponent<CollisionStay>();
				return (b, lr => b.Initialise(new CollisionStayTriggerData(i.Id, i.TagsToDetect,
					i.Listeners.ToActions(lr))));
			},

			[typeof(TriggerEnterTriggerInfo)] = (go, info, _, _, _) =>
			{
				var i = (TriggerEnterTriggerInfo)info;
				var b = go.AddComponent<TriggerEnter>();
				return (b, lr => b.Initialise(new TriggerEnterTriggerData(i.Id, i.TagsToDetect,
					i.Listeners.ToActions(lr))));
			},

			[typeof(TriggerExitTriggerInfo)] = (go, info, _, _, _) =>
			{
				var i = (TriggerExitTriggerInfo)info;
				var b = go.AddComponent<TriggerExit>();
				return (b, lr => b.Initialise(new TriggerExitTriggerData(i.Id, i.TagsToDetect,
					i.Listeners.ToActions(lr))));
			},

			[typeof(ConditionTriggerInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (ConditionTriggerInfo)info;
				var b = go.AddComponent<Condition>();
				return (b, lr => b.Initialise(new ConditionData(i.Id, i.Condition.Resolve(vr, cr),
					i.Listeners.ToActions(lr))));
			},

			[typeof(CameraInfo)] = (go, info, vr, cr, _) =>
			{
				var i = (CameraInfo)info;
				var b = go.AddComponent<CameraBehaviour>();
				return (b, lr => b.Initialise(new CameraData(i.Id, i.Listeners.ToActions(lr),
					i.View.Resolve(vr, cr), i.Size.Resolve(vr, cr))));
			},

			[typeof(SpawnerInfo)] = (go, info, vr, cr, spawner) =>
			{
				var i = (SpawnerInfo)info;
				var b = go.AddComponent<SpawnerBehaviour>();
				b.Spawner = spawner;
				return (b, lr => b.Initialise(new SpawnerData(i.Id, i.Listeners.ToActions(lr),
					i.TemplateId.Resolve(vr, cr), i.Position.Resolve(vr, cr))));
			},

			[typeof(DestroyInfo)] = (go, info, _, _, _) =>
			{
			...
			[typeof(VariableSetterInfo<Vector3>)] = (go, info, vr, cr, _) =>
			{
				var i = (VariableSetterInfo<Vector3>)info;
				var b = go.AddComponent<Vector3Setter>();
				return (b, lr => b.Initialise(new VariableSetterData<Vector3>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr), i.ValueToGet.Resolve(vr, cr))));
			},

			[typeof(VariableSetterInfo<int>)] = (go, info, vr, cr, _) =>
			{
				var i = (VariableSetterInfo<int>)info;
				var b = go.AddComponent<IntSetter>();
				return (b, lr => b.Initialise(new VariableSetterData<int>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr), i.ValueToGet.Resolve(vr, cr))));
			},

			[typeof(VariableSetterInfo<float>)] = (go, info, vr, cr, _) =>
			{
				var i = (VariableSetterInfo<float>)info;
				var b = go.AddComponent<FloatSetter>();
				return (b, lr => b.Initialise(new VariableSetterData<float>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr), i.ValueToGet.Resolve(vr, cr))));
			},

			[typeof(VariableSetterInfo<bool>)] = (go, info, vr, cr, _) =>
			{
				var i = (VariableSetterInfo<bool>)info;
				var b = go.AddComponent<BoolSetter>();
				return (b, lr => b.Initialise(new VariableSetterData<bool>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr), i.ValueToGet.Resolve(vr, cr))));
			},

			[typeof(VariableSetterInfo<string>)] = (go, info, vr, cr, _) =>
			{
				var i = (VariableSetterInfo<string>)info;
				var b = go.AddComponent<StringSetter>();
				return (b, lr => b.Initialise(new VariableSetterData<string>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr), i.ValueToGet.Resolve(vr, cr))));
			},
		};
		...
		{
			if (!Builders.TryGetValue(behaviourInfo.GetType(), out var builder))
				throw new ArgumentException($"Unsupported behaviour info type '{behaviourInfo.GetType()}'");

			return builder(gameObject, behaviourInfo, variableRegistry, compiledExpressionRegistry, entitySpawner);
		}
