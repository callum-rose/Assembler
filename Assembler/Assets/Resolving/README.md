...
Third stage of the YAML-to-game pipeline. Converts parsed `ValueSource<T>` instances (from `Assembler.Parsing`) into runtime `IValueProvider<T>` instances that behaviours read during gameplay. Also holds all per-run state registries (variables, assets, compiled expressions, entity transforms).
...
| `ValueResolver.Resolve<T>(ValueSource<T>, ResolutionContext)` | Extension method; single entry point for the conversion. Dispatches on `ValueSource` subtype. |
...
| `BehaviourData` / `TriggerData` | Base classes for per-behaviour resolved data bags (one subclass per behaviour). |

## Gotchas

- Depends on `Assembler.Parsing` (for `ValueSource<T>`, `BehaviourInfo`, etc.) and `Assembler.Compiler` (expressions).
