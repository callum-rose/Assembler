...
namespace Assembler.Behaviours
{
	public static class GameBehaviourFactory
	{
		private delegate (GameBehaviour, Action<IReadOnlyDictionary<BehaviourDescriptor, GameBehaviour>>) BehaviourBuilder(
			GameObject go, BehaviourInfo info, VariableRegistry vr, CompiledExpressionsRegistry cr, IEntitySpawner spawner, AssetRegistry ar);
		...
		private static readonly Dictionary<Type, BehaviourBuilder> Builders = new()
		{
			[typeof(BoxColliderInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new BoxColliderData(i.Id, i.Listeners.ToActions(lr),
					i.Size.Resolve(vr, cr, ar), i.IsTrigger.Resolve(vr, cr, ar))));
			},
			...
			[typeof(KeyHoldTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new KeyHoldTriggerData(i.Id, i.Key.Resolve(vr, cr, ar),
			...
			[typeof(KeyDownTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new KeyDownTriggerData(i.Id, i.Key.Resolve(vr, cr, ar),
					i.Listeners.ToActions(lr))));
			},
			...
			[typeof(KeyUpTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new KeyUpTriggerData(i.Id, i.Key.Resolve(vr, cr, ar),
					i.Listeners.ToActions(lr))));
			},
			...
			[typeof(TimerTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new TimerTriggerData(i.Id, i.Delay.Resolve(vr, cr, ar),
					i.Listeners.ToActions(lr))));
			},
			...
			[typeof(IntervalTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new IntervalTriggerData(i.Id, i.Interval.Resolve(vr, cr, ar),
					i.Listeners.ToActions(lr))));
			},
			...
			[typeof(ConditionTriggerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new ConditionData(i.Id, i.Condition.Resolve(vr, cr, ar),
					i.Listeners.ToActions(lr))));
			},
			...
			[typeof(CameraInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new CameraData(i.Id, i.Listeners.ToActions(lr),
					i.View.Resolve(vr, cr, ar), i.Size.Resolve(vr, cr, ar))));
			},
			...
			[typeof(SpawnerInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new SpawnerData(i.Id, i.Listeners.ToActions(lr),
					i.TemplateId.Resolve(vr, cr, ar), i.Position.Resolve(vr, cr, ar))));
			},
			...
			[typeof(VariableSetterInfo<Vector3>)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new VariableSetterData<Vector3>(i.Id, i.Listeners.ToActions(lr),
					i.ValueToSet.Resolve(vr, cr, ar), i.ValueToGet.Resolve(vr, cr, ar))));
			},
			...
			[typeof(SpriteInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new SpriteData(i.Id, i.Listeners.ToActions(lr),
					i.Sprite.Resolve(vr, cr, ar), i.Size.Resolve(vr, cr, ar))));
			},
			...
			[typeof(AudioSourceInfo)] = (go, info, vr, cr, es, ar) =>
			{
				...
				return (b, lr => b.Initialise(new AudioSourceData(i.Id, i.Listeners.ToActions(lr),
					i.Clip.Resolve(vr, cr, ar), i.PlayOnStart.Resolve(vr, cr, ar), i.Loop.Resolve(vr, cr, ar))));
			},
		};
			...
			AssetRegistry assets = null)
		{
			...
			return builder(gameObject, behaviourInfo, variableRegistry, compiledExpressionRegistry, entitySpawner, assets);
		}
