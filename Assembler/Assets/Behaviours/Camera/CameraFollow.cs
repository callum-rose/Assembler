...
namespace Assembler.Behaviours.Camera
{
	...
	/// <remarks>
	/// The follow/look-at targets and the priority are re-read every frame, so tag targets track entities spawned
	/// after build and a variable/expression-bound priority switches the live camera at runtime.
	...
	///   Priority: Virtual-camera priority; the brain shows the highest-priority live vcam. Re-read every frame, so binding it to a variable/expression dynamically switches which camera is live (Cinemachine blends across).
	...
	public class CameraFollow : GameBehaviour<CameraFollowData>
	{
		...
		// Re-resolve targets each frame so tag targets follow entities spawned after build, and re-read the priority
		// so a variable/expression-bound value switches the live camera at runtime (the brain blends to the winner).
		private void Update()
		{
			ApplyTarget(Data.Follow, t => _cam.Follow = t);
			ApplyTarget(Data.LookAt, t => _cam.LookAt = t);
			Data.Priority.UseIfValueExists(p =>
			{
				if (_cam.Priority.Value != p)
				{
					_cam.Priority = p;
				}
			});
