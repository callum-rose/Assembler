...
- **Guard sub-component creation in `OnInitialise`** (`if (_x == null) …`) — re-init must reuse, not stack a second Rigidbody/renderer/collider. Use `OnInitialise`, **not `Awake`**: `Awake`/`Start` don't run in edit mode, and the `validate-game` sandbox (`Builder.Instantiate`) and all EditMode tests build entities *without* entering play mode. A sub-component created in `Awake` (Rigidbody, renderer, collider, Light, child GameObject) therefore never exists there → `MissingComponentException`/NRE. The factory calls `OnInitialise` explicitly in both modes.
