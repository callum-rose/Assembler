...
{
	[TriggerOutputs("start", "position", "delta", "distance", "direction")]
		...
		ValueSource<float> MaxDuration) : BehaviourInfo(Id, Listeners)
	{
		public static SwipeTriggerInfo Create(string id,
			...
			new(id,
				listeners,
				ValueSourceFactory.CreateValueSource<float>(ctx, props.GetValueOrDefault("MinDistance")),
				ValueSourceFactory.CreateValueSource<float>(ctx, props.GetValueOrDefault("MaxDuration")));
