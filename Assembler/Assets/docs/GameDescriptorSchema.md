...
| `!if` | `ConditionalRefDto` | `!if { Condition, Then, Else }` (mapping) | A conditional value: yields `Then` when the boolean `Condition` reads true, else `Else`. All three may be any value (`!var`/`!expr`/literal/…); only the chosen branch is read each frame. Use as a setter's `Value` to express "set X to A if cond else B" without two condition-gated setters. |
